...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-bcx"
  !define PRODUCT_WEB_SITE "https://github.com/bitcoinxash/electrum-bcx"
  !define PRODUCT_PUBLISHER "bcx"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-bcx-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-bcx\electrum_bcx\gui\icons\electrum.ico"
...
Section
  ...
  File "c:\electrum-bcx\electrum_bcx\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bcx-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bcx-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-bcx-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-bcx-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-bcx-${PRODUCT_VERSION}.exe" 0


  ;Links bcx: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bcx" "" "URL:bcx Protocol"
  WriteRegStr HKCU "Software\Classes\bcx" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bcx" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bcx\shell\open\command" "" "$\"$INSTDIR\electrum-bcx-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\bcx"
