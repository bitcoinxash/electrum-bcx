...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-bcx-macos
	cd /tmp/electrum-bcx-macos
...
rm -rf /tmp/electrum-bcx-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-bcx-macos/image/
cp -r $1 /tmp/electrum-bcx-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-bcx_uncompressed.dmg \
    /tmp/electrum-bcx-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-bcx_uncompressed.dmg electrum-bcx-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-bcx_uncompressed.dmg

echo "Done."
sha256sum electrum-bcx-$VERSION.dmg
