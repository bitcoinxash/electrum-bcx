...
APPDIR="$BUILDDIR/electrum-bcx.AppDir"
...
APPIMAGE="$DISTDIR/electrum-bcx-$VERSION-x86_64.AppImage"
...
(
    ...
    pushd "$CONTRIB"/deterministic-build/electrum-big-locale
    if ! which msgfmt > /dev/null 2>&1; then
        echo "Please install gettext"
        exit 1
    fi
    for i in ./locale/*; do
        dir="$PROJECT_ROOT/electrum-bcx/$i/LC_MESSAGES"
        mkdir -p $dir
        msgfmt --output-file="$dir/electrum-bcx.mo" "$i/electrum-bcx.po" || true
    done
    popd
)


info "installing electrum-bcx and its dependencies."
...
cp "$PROJECT_ROOT/electrum-bcx.desktop" "$APPDIR/electrum-bcx.desktop"
cp "$PROJECT_ROOT/electrum_bcx/gui/icons/electrum.png" "$APPDIR/electrum.png"
